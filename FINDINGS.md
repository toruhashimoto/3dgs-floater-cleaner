...
# MRNF 高詳細での再検証（v2.0 既定設定 / 別データ RSOT64K, 2026-06-25）

v2.0 で既定を **MRNF・105k iter・max_cap=5M**（＋ディテール強化 `--use-error-map`/`--use-edge-map`）に変更したため、scale_reg の floater 低減がこの新レジームでも有効かを再検証。データ `260623_3GJ_RSOT64K`（1,050枚, init 179万点, RTX PRO 6000）。唯一変数=scale_reg、他は全一致。LichtFeld は seed 固定不可のため baseline 2本でノイズフロアを取得。
