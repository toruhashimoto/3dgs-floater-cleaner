...
# MRNF 高詳細での再検証（v2.0 既定設定 / 別の実機データセット, 2026-06-25）

v2.0 で既定を **MRNF・105k iter・max_cap=5M**（＋ディテール強化 `--use-error-map`/`--use-edge-map`）に変更したため、scale_reg の floater 低減がこの新レジームでも有効かを再検証。**別の実機 RealityScan データセット**（社内・1,050枚, init 179万点, RTX PRO 6000）。唯一変数=scale_reg、他は全一致。LichtFeld は seed 固定不可のため baseline 2本でノイズフロアを取得。

| run | scale_reg | floater(a) | 割合 | total gaussians | 効果 vs baseline平均 |
|---|---:|---:|---:|---:|---:|
| baseA | 0.0042 | 351 | 0.008% | 4.44M | —（baseline）|
| baseB | 0.0042 | 355 | 0.008% | 4.51M | ノイズフロア |
| reg02 | 0.02 | 263 | 0.005% | 5.00M | **−90（−25.5%）** |
| reg04 | 0.04 | 221 | 0.004% | 5.00M | **−132（−37.4%）** |

- ノイズフロア `|baseA−baseB| = 4`。reg02/reg04 の効果（−90 / −132）はノイズを大きく超え**有意**。scale_reg は **MRNF でも floater(a) を用量依存で低減**（方向は上段 MCMC と一致）。
- **ただし絶対量は極小**：MRNF 高詳細は baseline 時点で floater(a) が 0.008%（444万中 **351個**）とほぼ皆無。0.04 でも 221 個で、削減は数十〜百個規模。**長く高品質な MRNF 学習自体がフローターをほぼ消しており**、scale_reg が削る余地が小さい（30k で baseline floater が自然減する MCMC の傾向と整合）。
- floater(b)（凸包外）は treatment 側がやや多い（reg02 93,293 / reg04 90,054 vs baseline 平均 ~81,272）が、これは 5M 上限まで使い**総ガウシアンが +0.5M 多い**ため。大半は正当な背景構造で有害フローターの増加ではない。
- 学習時間 ~40–44分/本（105k, 5M cap, 1050枚, RTX PRO 6000 Blackwell）。

**結論：MRNF 高詳細でも scale_reg は floater(a) を低減（実在・用量依存・−25〜37%）。ただし MRNF 高詳細は baseline で既にフローターが極小なため、scale_reg の実用的な追加利得は限定的。フローター抑制の主役は MRNF 高詳細設定そのもの。** 強くフローターを断ちたい場合は scale_reg より撮り方・露出・ROI（floater(b) 側）の方が支配的、という下段の示唆がそのまま当てはまる。
