...
> `scale_reg=0.02` の floater **−75〜82%**（PSNR/SSIM ほぼ不変）は **MCMC での検証値**。既定の **MRNF 高詳細でも別データで再検証済み**で、scale_reg は floater(a) を用量依存で低減します（0.02 で **−25%**、0.04 で **−37%**、ノイズ比で有意）。ただし MRNF 高詳細は baseline 時点で floater(a) が極小（〜0.008%）のため**実用的な追加利得は限定的**です（詳細は [FINDINGS.md](FINDINGS.md)）。
...
- 幾何正則化 `scale_reg` でフローター抑制（強度：**標準 0.02** / 強め 0.04 / オフ）。MCMC −75〜82%／**MRNF 高詳細でも floater(a) 低減を再検証済み**（0.02 −25%・0.04 −37%。ただし baseline が極小で実用利得は限定的）
